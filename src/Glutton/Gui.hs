...
setup ucs updater window = do
  ...
  let sidebarContents = fmap (map $ mkSidebar updater activeFeed) ucs
  sidebar <- div
             # set id_ "sidebar"
...
mkSidebar :: Updater -> Element -> UnreadCount -> UI Element
mkSidebar updater activeFeed (n, c, h) = do
  let countString = if c > 0 then show c else ""
  feedSelector <- div #. "feedSelector"
                  #+ [ span # set text n
                     , span # set text countString #. "unreadCount" ]
  on click feedSelector $ const $ mkActiveFeed updater activeFeed h
  return feedSelector

mkActiveFeed :: Updater -> Element -> SubscriptionHandle -> UI ()
mkActiveFeed updater activeFeed handle = do
  sub <- liftIO $ S.get handle
  markAllReadButton <- span #. "button" # set text "♺ Mark All Read"
  deleteButton <- span #. "button" # set text "☢ Delete"
  header <- div #+
  ...
  on click markAllReadButton $ const $ do
    ...
    mkActiveFeed updater activeFeed handle

  on click deleteButton $ const $ do
    liftIO $ feedUrl <$> S.get handle >>= removeSubscription updater
    element activeFeed # set children []
...
mkItem sh i = do
  ...
  on click markRead $ const $ do
    ...
    _ <- element markRead # set text (check $ fromMaybe True newReadState)
    element link # set class_ (if fromMaybe True newReadState then "read" else "unread")
